...
function readColor(source: string, token: string): string {
  const match = source.match(new RegExp(`--color-${token}:\\s*(?<color>#[0-9A-Fa-f]{6})`));
  assert.ok(match?.groups?.color, `token ${token} deve existir no CSS`);
...
test("cores de texto pequenas atendem ao contraste WCAG AA", () => {
  ...
  const faint = readColor(source, "ink-faint");
